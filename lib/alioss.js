#!/usr/bin/env node
// From: https://github.com/staven630/webpack-oss/blob/master/lib/oss.js 
...
class AliOSS {
  constructor(options) {
    ...
    if (['accessKeyId', 'accessKeySecret', 'bucket', 'region'].some(key => !options[key])) {
      throw new Error(`请填写正确的 accessKeyId、accessKeySecret 和 bucket`)
    }

    if (this.config.format && !/[0-9]+/.test(this.config.format)) {
      throw new Error(`format 应为纯数字`)
    }
  ...
  getFileName(name) {
    const { config } = this
    const prefix = config.format ? path.join(config.prefix, config.format.toString()) : config.prefix
